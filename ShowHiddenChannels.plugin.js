 ...
 * @authorId 
 * @version 1.0
 * @description Savage on top js // Better ShowHiddenChannels 
 * @website yummy#2703
 * @source yummy#2703
 * @updateUrl yummy#2703
 */

module.exports = (_ => {
	const config = {
		"info": {
			"name": "Unlock Channel by Savage",
			"author": "Savage",
			"version": "1.0",
			"description": "Savage in top js"
		},
		"changeLog": {
			"fixed": {
				"Changelog >": "Fix Bug // Italian Version"
			}
		}
	};
	...
	return !window.BDFDB_Global || (!window.BDFDB_Global.loaded && !window.BDFDB_Global.started) ? class {
		...
		getDescription () {return `Il plug-in della libreria necessario per ${config.info.name} manca. Apri le Impostazioni del plugin per scaricarlo. \n\n${config.info.description}`;}
		
		downloadLibrary () {
			require("request").get("https://mwittrien.github.io/BetterDiscordAddons/Library/0BDFDB.plugin.js", (e, r, b) => {
				if (!e && b && r.statusCode == 200) require("fs").writeFile(require("path").join(BdApi.Plugins.folder, "0BDFDB.plugin.js"), b, _ => BdApi.showToast("Finished downloading BDFDB Library", {type: "success"}));
				else BdApi.alert("Errore "," Impossibile scaricare il plug-in della libreria BDFDB. Riprova più tardi o scaricalo manualmente da GitHub: https://mwittrien.github.io/downloader/?library");
			});
		}
		...
		load () {
			...
			if (!window.BDFDB_Global.downloadModal) {
				...
				BdApi.showConfirmationModal("Library Missing ",` Il plugin per la libreria necessario per <${config.info.name} manca. Fare clic su "Scarica ora" per installarlo.`, {
		...
		return class ShowHiddenChannels extends Plugin {
			onLoad () {
				...
				this.defaults = {
					settings: {
						sortNative:				{value: true, 	description: "Ordina i canali nascosti nell'ordine nativo invece di una categoria aggiuntiva"},
						showText:				{value: true, 	description: "Mostra canali di testo nascosti"},
						showVoice:				{value: true, 	description: "Mostra i canali vocali nascosti"},
						showAnnouncement:		{value: true, 	description: "Mostra canali di annunci nascosti"},
						showStore:				{value: true, 	description: "Mostra i canali del negozio nascosti"},
						showVoiceUsers:			{value: true, 	description: "Mostra gli utenti collegati nei canali vocali nascosti"},
						alwaysCollapse:			{value: false, 	description: "Comprimi sempre la categoria 'nascosta' dopo aver cambiato server"},
						showForNormal:			{value: true,	description: "Aggiunta di una voce di menu contestuale per la panoramica dell'accesso per i canali non nascosti"}
					}
				};
				...
				settingsItems.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.CollapseContainer, {
					...
					children: [
						...
						BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.SettingsItem, {
							...
							label: "Abilita per tutti i server",
							onClick: _ => {
						...
						BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.SettingsItem, {
							...
							label: "Disabilita per tutti i server",
							onClick: _ => {
